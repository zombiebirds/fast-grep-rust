...
fn search_set(idx: &fast_grep::persist::PersistentIndex, pattern: &str) -> Vec<(PathBuf, usize)> {
    let mut v: Vec<(PathBuf, usize)> =
        search_persistent_timed(idx, pattern, None, false, &[], &[], &[])
...
fn full_scan_set(tmp: &Path, pattern: &str) -> Vec<(PathBuf, usize)> {
    let mut v: Vec<(PathBuf, usize)> =
        search_full_scan(tmp, pattern, true, false, &[], &[], &[], false)
            .expect("full scan")
            .iter()
...
fn compact_clears_delta_files() {
    ...
    // Sanity: delta files exist before compact.
    {
        ...
        assert!(
            idx_pre.num_docs() > idx_pre.main_num_docs,
            "delta should be non-empty"
        );
        assert!(
            paths_exist(&idx_dir),
            "delta files should be present pre-compact"
        );
    }
    ...
    assert!(
        !paths_exist(&idx_dir),
        "delta files should be cleared after compact"
    );
...
fn compact_preserves_search_results() {
    ...
    assert_eq!(
        post_indexed, pre_full,
        "compact changed search results for 'beta'"
    );
...
fn compact_reclaims_deleted_docs() {
    ...
    assert!(
        idx_post.deleted_docs.is_empty(),
        "deleted set must be cleared"
    );
    ...
    assert!(
        hits.is_empty(),
        "deleted file's content must not appear: {hits:?}"
    );
    // The surviving file is still searchable.
    assert_eq!(
        search_set(&idx_post, "keep"),
        full_scan_set(tmp.path(), "keep")
    );
}
...
fn compact_on_fresh_index_is_noop() {
    ...
    assert_eq!(
        search_set(&idx, "alpha"),
        full_scan_set(tmp.path(), "alpha")
    );
}
...
fn compact_after_incremental_with_modified_file() {
    ...
    fs::write(
        tmp.path().join("a.txt"),
        "second version with marker PATTERN\n",
    )
    .unwrap();
